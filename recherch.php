...
<html>
	...
	<body>
		<?php
			function recherche($rire){
				include('menu.php');
				$res = array();
				for ($i=0; $i < count($voiture) ; $i++) { 
					if ($voiture[$i]['caractere']==$rire) {
						array_push($res, $i);
					}
				}
				return $res;
			}
			$post= recherche($_POST['nom']);
			if (count($post) > 0) {?>
			 	<div class="home2">
					<header>
...
+							$indice = $post[$i];
							...
							<div class="image1">
								<?php
									echo "<img src='voiture/".$voiture[$indice]['image'].".jpg'>";
								?>
							</div>
							<div class="centre2">
								<?php 
									...
									<strong><?php echo $voiture[$indice]['caractere']; ?></strong><br><br>
									<span class="nom"><?php echo $voiture[$indice]['nom'];?></span><br><br>
									<b><?php echo $voiture[$indice]['prix'];?></b><br><br>	
